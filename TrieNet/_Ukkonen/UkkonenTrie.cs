...
namespace Gma.DataStructures.StringSearch
{
    public class UkkonenTrie<T> : ITrie<T>
    {
        ...
        public void Add(string key, T value)
        {
            ...
            ReadOnlySpan<char> remainder = key.AsSpan();
            Node<T> s = _root;
            ...
            for (var i = 0; i < remainder.Length; i++)
            {
                ...
                Node<T> activeNode = Update(s, text, remainder.Slice(i), value, out ReadOnlySpan<char> sp);
                // line 8: make sure the active Tuple is canonical
                s = Canonize(activeNode, sp, out ReadOnlySpan<char> t);
                text = t.ToString();
            }

            // add leaf suffix link, is necessary
            if (_activeLeaf.Suffix is null && _activeLeaf != _root && _activeLeaf != s)
        ...
        private bool TestAndSplit(Node<T> inputs, ReadOnlySpan<char> stringPart, char t, ReadOnlySpan<char> remainder, T value, out Node<T> node)
        {
            ...
            if (remainder.Equals(e.Label.AsSpan(), StringComparison.Ordinal))
            {
            ...
            if (remainder.StartsWith(e.Label) || !e.Label.AsSpan().StartsWith(remainder))
                return true;
         ...
         */
        private static Node<T> Canonize(Node<T> s, ReadOnlySpan<char> inputstr, out ReadOnlySpan<char> outStr)
        {
            if (inputstr.IsEmpty)
            {
                outStr = inputstr;
            ...
            outStr = inputstr;
            Edge<T> g = s.GetEdge(outStr[0]);
            // descend the tree as long as a proper label is found
            while (g != null && outStr.StartsWith(g.Label))
            {
                outStr = outStr.Slice(g.Label.Length);
                currentNode = g.Target;
                if (outStr.Length > 0)
                {
                    g = currentNode.GetEdge(outStr[0]);
                }
            }
         ...
         * @param value the value to add to the index
         */
        private Node<T> Update(Node<T> inputNode, ReadOnlySpan<char> stringPart, ReadOnlySpan<char> rest, T value, out ReadOnlySpan<char> sp)
        {
            Node<T> s = inputNode;
            ReadOnlySpan<char> tempstr = stringPart;
            char newChar = stringPart[^1];
            ...
            while (!endpoint)
            {
                ...
                {
                    var canret = Canonize(s.Suffix, SafeCutLastChar(tempstr), out ReadOnlySpan<char> spa);
                    s = canret;
